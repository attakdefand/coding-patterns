...
fn main() {
    ...
    match three_sum_safe(&nums, 0) {
        Some((i, j, k)) => println!("three_sum_safe: Found indices {}, {}, and {} for target 0", i, j, k),
    ...
    println!("container_with_most_water_safe: Maximum area is {}", max_area);
    
    // Demonstrate find_subarray_sum_safe
    let nums = vec![1, 4, 2, 7, 3];
    match find_subarray_sum_safe(&nums, 6) {
        Some((start, end)) => println!("find_subarray_sum_safe: Found subarray from index {} to {} for target 6", start, end),
    ...
    match two_sum_safe(&nums, target) {
        Some((i, j)) => println!("two_sum_safe with overflow risk: Found indices {} and {}", i, j),
        None => println!("two_sum_safe with overflow risk: Handled gracefully (no crash)"),
    }
    
    let height = vec![i32::MAX, i32::MAX];
    let max_area = container_with_most_water_safe(&height);
    println!("container_with_most_water_safe with overflow risk: Result is {} (handled gracefully)", max_area);
